...
//*                                    Data                                    */
//* -------------------------------------------------------------------------- */
// * Sample Data don't Delete
// import { websiteData } from '../../data/data';

// * This use to manipulate Data
...
import CreateWebModal from '../../components/ModalWebsite/CreateWebModal';
import SkeletonCard from '../../components/Cards/SkeletonCard';
import { Filter, Limit, Search, Sort, Pagination } from '../../components';
...
const AdminWebsites = () => {
  ...
  // const websites = useSelector((state) => state.auth.webs);
  ...
  // }, []); // eslint-disable-line react-hooks/exhaustive-deps
	...
	// 		// * Send to Server
	// 		await fetch(`${process.env.REACT_APP_BASE_URL}/webs?search=${search}`, {
	// 			method: "GET",
	...
	const handleFilter = () => {
		...
		// // Extract the genre
		// rtkData.webs.map((item, index) => {
		// 	// console.log(item.genres),
		// 	extractedFilter.push(item.genres)
		// })

		// // Flatten the array of arrays into a single array [{a,b},{c,d}] => [{a,b,c,d}]
		// extractedFilter = extractedFilter.flat(); 

		// // Remove duplicates using Set
		// extractedFilter = Array.from(new Set(extractedFilter));

		// if(extractedFilter.length > filterResult) {
		// 	// Convert into 2D Array
		// 	for(var i = 0; extractedFilter.length > i; i++) {
		// 		filterResult.push({ 
		// 			name: extractedFilter[i], 
		// 			filter: false
		// 		})
		// 	}
		// }
		...
		let extractedFilter = rtkIsSuccess ? rtkData.webs.flatMap(item => item.genres) : [];
		...
		try{
			localStorage.setItem('filterWebsData', JSON.stringify(filterResult));
		} catch(error) {
			console.log(error);
		}

		// Load data from local storage on component mount
		const savedData = localStorage.getItem('filterWebsData');
	...
	const handleClearFilter = () => {		
		...
		// * Set All Filter from local storage into False
  ...
  useEffect(() => {
	...
	}, [websiteAdded]); // eslint-disable-line react-hooks/exhaustive-deps
	...
	}, [websiteAdded, reloadFilter]); // eslint-disable-line react-hooks/exhaustive-deps
	...
	// * Divide all the data into page size
	...
	return (
		...
		>			
			...
			<CreateWebModal
				...
				setWebsiteAdded={ setWebsiteAdded }
			/>
				...
				{/* {
					rtkIsLoading ?
						websites.map((item, index) => (
							<SkeletonCard key={index+item._id} />
						))
					:
						searchResult.length > 0 ?
							searchResult
							// .filter(data => data.cetegory === filter)
							.map((item, index) => (
								<ProjectsItem
									key={index+item._id}
									id={item._id}
